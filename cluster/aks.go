...
func (c *AKSCluster) UpdateCluster(request *pkgCluster.UpdateClusterRequest, userID uint) error {
	...
	// AKS currently only supports a single node pool
	// For more info, see: https://feedback.azure.com/forums/914020-azure-kubernetes-service-aks/suggestions/34917127-support-multiple-node-pool
	if len(request.AKS.NodePools) > 1 {
		return fmt.Errorf("Too many node pools: %d", len(request.AKS.NodePools))
	}
